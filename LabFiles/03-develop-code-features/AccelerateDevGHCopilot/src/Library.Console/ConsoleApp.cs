...
public class ConsoleApp
{
    ...
    static void WriteInputOptions(CommonActions options)
    {
        ...
        {
            Console.WriteLine(" - \"b\" per verificare la disponibilità dei libri");
        }
    ...
    async Task<ConsoleState> SearchBooks()
    {
        ...
        var book = _jsonData.Books!
            .FirstOrDefault(b => string.Equals(b.Title, bookTitle, StringComparison.OrdinalIgnoreCase));
        ...
        if (bookItem == null)
        {
            Console.WriteLine($"No copy is available for \"{book.Title}\".");
            return ConsoleState.PatronDetails;
        }

        var activeLoan = _jsonData.Loans!
            .FirstOrDefault(loan => loan.BookItemId == bookItem.Id && loan.ReturnDate == null);

        if (activeLoan == null)
        {
            Console.WriteLine($"{book.Title} è disponibile per il prestito");
        }
        else
        {
            Console.WriteLine($"{book.Title} è in prestito a un altro utente. La data di scadenza prevista è {activeLoan.DueDate}");
        }
